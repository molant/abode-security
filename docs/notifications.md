...
The Abode Security panel (`/abode_security`) includes a **Cameras** tab that auto-discovers every camera entity sharing an HA device with at least one Abode-managed sensor. The tab appears for all users without any configuration.

When you tap a snapshot notification generated by the blueprint or the minimal automation above, the Companion app opens `/abode_security?tab=cameras&camera=<entity_id>` — the panel loads directly on the Cameras tab and scrolls/highlights the triggering camera so you can see its current state immediately.

Cameras refresh as still images every 5 seconds while the tab is visible (the `/api/camera_proxy/` endpoint is used, which is already auth-gated by your HA session). Live stream is not part of this release — the still-image view covers the "check what's happening now" use case with no streaming dependencies.
