...
- `scheduling/state_machine.py` — `derive_state` pure function: pair is `ARMED` iff `last_armed_at > last_disarmed_at` and `now ≤ expected_disarm_at`. No side effects. `expected_disarm_at` computes the next occurrence of `disarm_time` ≥ `last_armed_at` in HA's local tz (DST-safe). `in_arm_window` answers the mirror question — is *this instant* inside one of a pair's windows — anchored on the most recent `arm_time` occurrence, which is also the weekday it matches against (#212).
...
- `scheduling/manager.py` — `ScheduleManager`: CRUD + full runtime. Registers one daily `ScheduleClock` handle per pair for the arm edge; disarm is a one-shot `async_call_later`, and four things create one: a successful arm, the `already_home` skip, startup reconciliation, and the override listener adopting a panel that entered Home mid-window (#212). (`async_update` calls `_schedule_disarm` too, but only ever to *re*-register a timer it found pending.) Reconciliation and the override listener both defer via `_defer_until_panel_exists` when the panel is not yet in `hass.states` — which is every setup — waiting on `EVENT_HOMEASSISTANT_STARTED` during startup and on the panel entity appearing thereafter — including after a startup miss, which hands off to the same subscription (see the reload entry below). Fires `abode_security.schedule_fired / .schedule_skipped / .schedule_failed` events. Raises `schedule_fire_failed` repair issue after retry exhaustion; clears it on next success.
...
One gap is deliberately left open: a dropout that outlasts the window. If the panel is still unavailable at `disarm_time`, `_disarm_impl` takes its conservative `panel_unavailable` branch, stamps `last_disarmed_at`, and does not re-attempt when the panel returns. #212 did not close it: the blip guard never moves the remembered mode off `armed_home`, so a recovery is not a transition *into* Home and the adoption path below is not consulted — and by then the window has closed, so it would refuse anyway. `test_unavailable_across_disarm_time_skips` pins that landing.

**The listener is bidirectional: it gives the panel up when it leaves Home, and takes it on when it arrives** (#212). The arm and disarm edges are asymmetric by design — `_register_pair_timers` registers only the daily *arm* callback, and until #212 the one-shot disarm was created by only three things: a successful arm, the `already_home` skip, or reconciliation rebuilding one after a restart. Adoption is the fourth, and the rest of this section is about what happened before it existed. So a pair whose arm edge hit the `away_active` branch got no disarm timer, and nothing re-evaluated afterwards: the scheduler read panel state at the arm instant and at startup, and nowhere else. A night that began in Away therefore stranded the panel armed through the whole of the next day, however it moved in between — the reported case was an unrelated presence bug tripping vacation mode at 22:32, a `23:00` arm skipped `away_active`, a manual arm to Home at 23:37, and a household waking to `armed_home` at 07:25.

The tempting fix — schedule the disarm on the `away_active` path too — is a security regression, and is not what changed: on a genuine trip it plants a timer that unarms an empty house at 06:00. Refusing to pre-commit to a disarm while the panel is in Away is correct, and so is the arm-time rule (schedule the disarm only on a successful arm, or when the panel is already in the mode the schedule would have set). What was missing is the *other* instant at which the same question can be asked safely. `_on_panel_state_changed`'s `armed_home` early return is now the mirror of the leave-Home edge beneath it: a transition into `armed_home` from a non-scheduler context runs `_handle_manual_arm`, which adopts every enabled pair whose arm→disarm window contains the moment and registers the one-shot disarm.

Six conditions keep adoption from ever scheduling a disarm nobody asked for. The panel must not have *left*, re-read at the top of *every* iteration rather than sampled once for the pass — and tested for `armed_away`/`disarmed` by name, not for "not Home", since a match there abandons the whole pass and a blip would otherwise strand every pair behind the one being written — `_persist_runtime` awaits a real `Store.async_save`, so a multi-pair adoption is not atomic. The pair must be enabled. `now` must be *in window*. It must not already own the panel (`derive_state` reports `ARMED`), which leaves a pair the scheduler armed alone and leaves a pair left `ARMED`-without-a-handle by a reload to reconciliation, whose rules for that case are deliberately stricter. And `derive_state` is checked once more *after* the write settles: the panel can leave Home inside that suspension, and if `_handle_manual_override` got its write in first it has stamped `last_disarmed_at` past our anchor, so the merged record reads `IDLE`. The pre-write guard cannot catch that one, because the loser of the race has not written yet when the winner reads — though this closes only one side of it, since the override is dispatched as a task and may equally resume second. Finally the panel is read once more after the reason write (below), before the timer, so that second suspension leaves the orphan window no wider than one write did. That last guard tests for `armed_away`/`disarmed` by name rather than for "not Home", and the distinction is #212 in miniature: an availability blip must be let *through*, because the #216 guard in `_on_panel_state_changed` returns on a blip before dispatching the override and before updating `_last_panel_state`, so nothing stamps `last_disarmed_at` and recovery is not an edge — declining there would leave the pair `ARMED` with no handle until the next restart, stranding exactly the pair adoption exists to rescue. Letting it through installs a timer that `_disarm_impl` re-checks at fire time, which is what the pre-split single write did.

A seventh condition lives in the caller, and it is the one doing the most work: `_on_panel_state_changed` dispatches here only on the *edge* into Home, never on a repeat `armed_home` event. `AbodeAlarm._update_callback` calls `schedule_update_ha_state()` on every SocketIO device event and its `extra_state_attributes` carry the backup flags, so armed_home → armed_home events arrive all night. That condition is the whole of what separates them from someone actually arming the panel, and `test_an_attribute_refresh_on_a_home_panel_is_not_an_arrival` is what stops a refactor deleting it quietly.

`previous is None` is excluded as well, and the reason is narrower than it looks. It is **not** that reconciliation picks the case up: `async_reconcile_on_startup` only rebuilds timers for pairs that are already `ARMED`, and the `away_active` branch stamps `last_disarmed_at` without ever setting `last_armed_at`, so a #212-shaped pair fails both of its guards (`last_armed_at is None`, then `last_disarmed_at >= last_armed_at`). The exclusion is plain conservatism instead: a `None` seed means the panel was unavailable when the listener started and this is the first mode it has ever reported, so there is no evidence it *entered* Home rather than having been there all along — and adopting on an inference rather than an observation is not a trade worth making on this path.

**That leaves a second residual gap, sibling to the dropout one above.** A reload or restart that lands while the panel is unavailable, on a night whose arm was skipped `away_active`, strands the panel for the rest of the window: adoption declines because it never witnessed a transition, and reconciliation declines because the pair was never `ARMED`. It fails in the safe direction — the panel stays armed, never unarmed — and `test_first_ever_panel_reading_is_not_a_transition` pins the landing so closing it later is a deliberate change rather than an accident. Closing it would mean teaching reconciliation to consider in-window pairs with no live anchor, which is a wider change than #212 called for.

**What is deliberately *not* a guard is whether `_pending_handles` already holds a disarm for the pair.** It reads like the obvious check and is actively wrong: a fired one-shot used to leave its entry behind — `_disarm_impl` never touched the dict — so the key survived every completed disarm, and the dict quietly stopped meaning "a disarm is pending" and started meaning "a disarm was scheduled at some point since the last reload". Guarding on it therefore worked exactly once per reload: the night after any normal disarm, the stale entry swallowed the adoption and #212 reappeared in full. `_clear_spent_disarm_handle` now clears the entry from inside both one-shot callbacks, which also stops `async_update`'s `had_pending_disarm` re-registering a timer for a window that closed hours ago. But adoption still does not consult it, because it does not need to: `derive_state` covers the pair-owns-the-panel case, and for everything else `_set_disarm_handle` already cancels whatever it replaces, so two arrivals in quick succession converge on one timer at the right instant rather than double-booking.

`in_arm_window` (in `state_machine.py`) anchors on the most recent occurrence of `arm_time` at or before `now`, and matches `pair.weekdays` against *that* occurrence's weekday rather than `now`'s. The two differ every morning of an overnight pair: Tuesday 02:00 sits squarely inside the Monday 23:00→06:00 window, and checking `now.weekday()` would call it outside. Anchoring on the arm edge is also what makes the every-weekday case work, where the weekday test can never reject — the `expected_disarm_at` comparison is then the only thing separating "inside the window" from "after last night's closed", because at 21:00 the anchor is *yesterday* 23:00 and its boundary (today 06:00) is already past.

Fall-back nights need two separate pieces of care there, and getting either wrong reported a window the panel was inside as closed. The anchor pins `fold=0` rather than inheriting it from `now`, because the daily arm timer fires on the *first* pass through a repeated hour, so that is the pass the anchor has to name. And both comparisons are made between absolute instants (`.astimezone(UTC)`), because PEP 495 makes `<`/`>` between two datetimes sharing a `tzinfo` a naive wall-clock comparison that ignores `fold` entirely — so an 02:30 CEST that was already an hour in the past still compared as *later* than 02:00 CET, rolling the anchor back a full day. `TestInArmWindow` pins both against Europe/Madrid's 2030-10-27. Spring-forward is pinned as a decision rather than a fix: on a skipped arm hour `async_track_time_change` does not fire the arm at all while `in_arm_window` reports the window open, which is benign — adoption can only ever schedule a disarm, and only for a panel already in Home.

Adoption reuses `already_home` rather than growing a seventh skip reason: it is the same physical situation as the arm-edge branch of that name — panel found in Home, take ownership so our disarm fires later — reached at a different instant, and the documented set stays closed. It is persisted in a *second* `_persist_runtime` call, after the post-write confirmation rather than alongside the `last_armed_at` anchor: written together, a declined adoption left `already_home` on a pair it had not adopted, overwriting the `manual_override` the winner of that race had just written and leaving a `schedules/list` row that read `IDLE` while claiming the scheduler had taken the panel on. The panel is re-read synchronously after the second write, before the timer, so that write's own suspension does not widen the harmless-orphan window. One overlap is left alone as cosmetic: a user arming Home *during* a scheduled arm's confirmation poll trips both paths, so the pair reports `schedule_skipped(already_home)` as well as `schedule_fired`. `_set_disarm_handle` cancels whichever timer it replaces, so exactly one disarm survives and it is at the right instant.

**The two edge handlers are serialised, in event order** (#212). Both are dispatched as tasks by `_on_panel_state_changed` and both await per pair, so a rapid Home → Away → Home used to interleave them: the adoption reached a pair the override had not got to yet, saw it still `ARMED` and skipped it as already owned, and the override then cancelled that pair's handle and marked it `IDLE`. The panel ended Home, in-window, with a timer-less pair and no later edge coming to re-adopt it — #212's own symptom, one pair over from the fix for it, and reachable with two or more in-window pairs. `_edge_lock` (an `asyncio.Lock` held for each handler's whole pass) is what closes it; acquisition is FIFO and the tasks are created in event order, which is what makes it *event order* rather than merely one-at-a-time. The claim is scoped to those two handlers. `async_reconcile_on_startup` is deliberately *not* under the lock, and `async_setup` starts the listener before awaiting it, so an edge can land inside reconcile's per-pair awaits. What that does *not* do is strand or mis-anchor a timer: reconcile re-reads each pair and computes `expected_disarm_at` from the anchor it finds with no await in between, and `_persist_runtime` mutates the cached record before its own await, so reconcile always sees an in-flight adoption's anchor rather than a pre-restart one. Whichever order the two arrive in, the surviving handle is anchored on the current `last_armed_at`. The residue is cosmetic and is a reason-clobber of the same class as the `already_home`/`manual_override` one above: reconcile samples `panel_str` *once*, so a panel that arrives Home mid-pass leaves it writing `reconcile_panel_not_home` and a `last_disarmed_at` that predates the adoption's anchor — the pair stays ARMED and keeps its adopted timer, but the `schedules/list` row reports a skip reason from a panel reading that is already stale. `test_an_override_cannot_interleave_with_a_later_adoption` pins the serialisation, and needs both a stalled store write (to produce the overlap) and a clock advanced inside that stall (because `FakeClock` is frozen, and two handlers stamping the same instant would fail `derive_state`'s strict comparison for a harness reason instead).

**A flapping mode source now costs churn it did not before, and that is the accepted side of a trade.** Before adoption existed, a panel oscillating Home → Away → Home inside a window cost one store write in total: the first leave-Home edge marked the pair `IDLE` and every later one was a `derive_state` no-op. Each full cycle now re-arms the pair, so it costs three (immediate, non-debounced) `Store.async_save` calls — two for the adoption, anchor then reason, and one for the release — and two `schedule_skipped` events — and that event is a documented notification surface, so a presence automation flapping at 01:00 sends a notification pair per flap. The end state is always consistent; this is churn, not corruption. Note that it is bounded *per cycle*, not per unit time — a source flapping every few seconds writes the store every few seconds. The obvious guard — refuse adoption when the pair was last released by `manual_override` inside this same window — was rejected because it cannot tell a flap from the legitimate flow it resembles: disarm by hand at 01:00, re-arm Home at 01:30, still want the 06:00 release. Re-adopting a genuine leave-and-return is the behaviour worth having, and it is exactly what #212 is about. `test_a_flapping_mode_source_re_adopts_every_cycle` pins the cost so a future guard is a deliberate choice rather than a silent one.
...
Both deferrals go through it, because `async_reconcile_on_startup` had the same trap and the worse consequence. `_register_all_timers` only restores the daily *arm* callback — reconciliation is the only thing that rebuilds a one-shot **disarm** — so a reload landing mid-window left the panel armed with nothing scheduled to disarm it at all, which is #216's own symptom reached by the sibling path. `_schedule_disarm` is normally called by the arm edge (and by the `already_home` skip; `async_update` also calls it, but only ever to *re*-register a timer it found pending), but after a reload none of those have run. For the pair this paragraph is about — one still `ARMED` mid-window — reconciliation remains the only thing that rebuilds the handle, because #212's adoption path explicitly refuses a pair that already owns the panel. (Adoption is a second route to a rebuilt handle after a reload, but only for an in-window pair that is *not* `ARMED`, and only once it witnesses an edge into Home.) So without reconciliation the override listener would have had nothing to cancel. Fixing one without the other would have been half a fix. The two are not symmetrical in what a *missing* panel means, though. Reconciliation still has work to do without one — its conservative `reconcile_panel_not_home` branch is what stops `last_armed_at` from sitting ahead of `last_disarmed_at` forever — so it passes `on_missing_panel=RUN_WITHOUT_PANEL` and runs anyway: immediately on the startup retry, and via a `PANEL_WAIT_TIMEOUT` backstop on the state-added path, which would otherwise wait for an entity that is never coming. That timeout is floored at HA's own `SLOW_SETUP_MAX_WAIT` (the test asserts `>=`, not equality — overshooting is harmless, undershooting is not): platforms forward in milliseconds in practice, but HA permits them 300 s, and a backstop that fires first would stamp `last_disarmed_at` against a panel that was merely late — reintroducing the stranding by a third route. The listener has nothing to listen to, so it does not proceed — but it does keep waiting, on both triggers, and it says so once at the backstop, because a manual-override listener that is disabled on a security integration is the wrong thing to be quiet about. That symmetry is why the startup retry hands off rather than returning: `EVENT_HOMEASSISTANT_STARTED` fires immediately *after* `hass.state` becomes `running`, so a miss there can install the same state-added subscription instead of leaving the listener dead until the next restart. The only cost is that the panel-less account hears about it `PANEL_WAIT_TIMEOUT` later than it used to.
...
- If the *user* manually arms Home during the confirmation poll, the pair now reports **both** `schedule_skipped` with `reason=already_home` and `schedule_fired` — since #212 the arm's own path and the adoption path both fire for that one physical situation. `_arm_impl` proceeded because the panel read `disarmed`, so the remembered mode is `disarmed`, and the user's arm carries a foreign context: the edge into Home is genuine as far as the listener can tell. Only the events double up; `_set_disarm_handle` cancels whichever timer it replaces, so exactly one disarm survives and it is at the right instant. See the adoption section's overlap note above.
